...
APP_NAME="AI Lead Outreach CLI"; DEFAULT_DB="outreach.db"; DEFAULT_CONFIG="config.json"; DEFAULT_DAILY_LIMIT=10
ALLOWED_STATUSES={"NEW","QUALIFIED","DRAFTED","APPROVED","CONTACTED","REPLIED","INTERESTED","MEETING","WON","LOST","DO_NOT_CONTACT"}
PHONE_RE=re.compile(r"^\+?[0-9][0-9]{6,14}$"); EMAIL_RE=re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")
@dataclass
class Lead:
...
def load_config(path=DEFAULT_CONFIG):
    defaults={"database":{"path":DEFAULT_DB},"outreach":{"daily_limit":10,"dry_run":True},"messaging":{"provider":"disabled"},"qualification":{"weights":{"decision_maker":20,"service_fit":20,"painpoint":20,"contactability":15,"company_fit":10,"digital_opportunity":10,"data_quality":5}}}
    p=Path(path)
...
def score_lead(l):
    r=qualify_lead(l); return r.score,list(r.reasons)
...
def main():
    ...
    try:
        ...
        elif a.command=="leads":show_leads(c,a.status)
