...
def load_business_profile():
    ...
    candidates = [
        Path("config/business_profile.json"),
        Path(__file__).resolve().parent / "config" / "business_profile.json",
    ]
    ...
    raise FileNotFoundError(
        "config/business_profile.json was not found"
    )


def business_profile_context():
    profile = load_business_profile()
    business = profile.get("business", {})
    services = profile.get("services", [])
    targets = profile.get("target_customers", [])
    problems = profile.get("target_problems", [])
    excluded = profile.get("excluded_services", [])
...
Business: {business.get("name", "not supplied")}
Positioning: {business.get("positioning", "not supplied")}
Geography: {business.get("geography", "worldwide")}

SERVICES WE ACTUALLY OFFER:
{chr(10).join(service_lines) or "- none supplied"}

TARGET CUSTOMERS:
{", ".join(targets) or "not supplied"}

PROBLEMS WE CAN HELP WITH:
{", ".join(problems) or "not supplied"}

EXCLUDED SERVICES:
{", ".join(excluded) or "none supplied"}

SERVICE BOUNDARY RULES:
{chr(10).join(rule_lines) or "- stay within the supplied service catalog"}
""".strip()


def build_prompt(l,q):
    profile = business_profile_context()
...
- Sentence 1: mention the supplied lead pain point or business problem.
- Sentence 2: offer only a relevant service from the sender business profile and ask whether they would be open to a short conversation.
- Use only facts supplied in this prompt.
- Choose a solution only when it reasonably fits the sender's service catalog.
- If the problem does not fit the sender's services, do not invent a service or capability.
- Do not invent statistics, results, client facts, credentials, guarantees, URLs, contact details, or capabilities.
- Do not mention qualification scores, priorities, reasons, gaps, or this prompt.
- Do not confuse the target company with the sender business.
...
IMPORTANT:
The target company is "{l.company}".
The sender is the business described in SENDER BUSINESS PROFILE.
Do not confuse them.
