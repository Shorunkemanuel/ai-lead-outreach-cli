...
class Milestone2QualificationTests(unittest.TestCase):
    ...
    def test_m2_columns_and_indexes_exist(self):
        ...
        self.assertTrue({
            "qualification_score", "qualification_priority", "qualification_reasons",
            "qualification_gaps", "qualified_at"
        }.issubset(columns))
    ...
    def test_high_qualification(self):
        lead = lead_cli.Lead(
            company="High Fit Co", contact_name="Ada", job_title="CEO",
            phone="+2348012345678", email="ada@example.com",
            website="https://example.com", industry="Software", country="Nigeria",
            employees="10", painpoint="Poor mobile UX causes checkout abandonment",
            source="Apollo",
        )
    ...
    def test_medium_qualification(self):
        lead = lead_cli.Lead(
            company="Medium Fit Co", job_title="Manager", phone="+2348012345678",
            website="https://example.com", industry="Software", employees="10",
    ...
    def test_low_qualification(self):
        lead = lead_cli.Lead(
            company="Low Fit Co", phone="+2348012345678", industry="Software",
            employees="10", painpoint="Poor mobile UX",
        )
        result = lead_cli.qualify_lead(lead)
    ...
    def test_score_is_always_bounded(self):
        leads = [
            lead_cli.Lead(company="Empty"),
            lead_cli.Lead(
                company="Full", contact_name="Ada", job_title="CEO",
                phone="+2348012345678", email="ada@example.com",
                website="https://example.com", industry="Software", country="Nigeria",
                employees="10", painpoint="Poor mobile UX causes checkout abandonment",
                source="Apollo",
    ...
    def test_reasons_and_gaps_are_explainable(self):
        lead = lead_cli.Lead(
            company="Explainable Co", job_title="CEO", phone="+2348012345678",
            painpoint="Manual lead scoring takes hours",
        )
        result = lead_cli.qualify_lead(lead)
        self.assertIn("decision maker identified", result.reasons)
        self.assertIn("phone available", result.reasons)
        self.assertIn("email available", result.gaps)
    ...
    def test_qualification_is_persisted_to_sqlite(self):
        ...
        self.connection.execute(
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,'NEW',?,?)""",
            (
                "Persist Co", "Ada", "CEO", "+2348012345678", "ada@example.com",
                "https://example.com", "Software", "Nigeria", "10",
                "Poor mobile UX", "test", now, now,
            ),
        )
        ...
        row = self.connection.execute(
            "SELECT qualification_score,qualification_priority,qualification_reasons,qualification_gaps,qualified_at,status FROM leads"
        ).fetchone()
    ...
    def test_qualification_is_deterministic(self):
        lead = lead_cli.Lead(
            company="Stable Co", job_title="CEO", phone="+2348012345678",
            email="ceo@example.com", website="https://example.com",
            industry="Software", employees="10", painpoint="Poor mobile UX",
        )
    ...
    def test_priority_filter_outputs_only_requested_priority(self):
        ...
        self.connection.executemany(
            """INSERT INTO leads
            (company,job_title,phone,email,website,industry,employees,painpoint,status,created_at,updated_at)
            VALUES (?,?,?,?,?,?,?,?,'NEW',?,?)""",
            [
                ("High Co", "CEO", "+2348012345678", "ceo@high.com", "https://high.com", "Software", "10", "Poor mobile UX causes checkout abandonment", now, now),
                ("Skip Co", "", "+2348012345679", "", "", "", "", "", now, now),
            ],
        )
        self.connection.commit()
        lead_cli.qualify_all(self.connection, lead_cli.load_config("/nonexistent/config.json"))
        ...
        with redirect_stdout(output):
            lead_cli.show_qualified(self.connection, "high")
    ...
    def test_existing_m1_lead_survives_qualification(self):
        now = lead_cli.utc_now()
        self.connection.execute(
            "INSERT INTO leads(company,phone,email,status,created_at,updated_at) VALUES(?,?,?,?,?,?)",
            ("M1 Existing Co", "+2348012345678", "old@example.com", "NEW", now, now),
        )
        self.connection.commit()
        ...
        self.assertEqual(
            self.connection.execute("SELECT company FROM leads").fetchone()[0],
            "M1 Existing Co",
        )
