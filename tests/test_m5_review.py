...
class Milestone5HumanReviewTests(unittest.TestCase):
    def setUp(self):
        ...
        self.connection.execute(
            ...
            """,
            (
                1, "GENERATED", message, "test-model", "test prompt", "",
    ...
    def test_generated_draft_can_be_approved(self):
        ...
        row = self.connection.execute(
            "SELECT status, reviewed_at FROM outreach_drafts WHERE id=1"
        ).fetchone()
    ...
    def test_approval_does_not_send(self):
        m4_outreach.approve_draft(self.connection, 1)
        row = self.connection.execute(
            "SELECT status FROM outreach WHERE lead_id=1"
        ).fetchone()
    ...
    def test_approval_is_audited(self):
        m4_outreach.approve_draft(self.connection, 1)
        row = self.connection.execute(
            "SELECT event, detail FROM outreach_events WHERE draft_id=1 ORDER BY id DESC LIMIT 1"
        ).fetchone()
    ...
    def test_rejected_draft_is_not_approved(self):
        ...
        row = self.connection.execute(
            "SELECT status FROM outreach_drafts WHERE id=1"
        ).fetchone()
    ...
    def test_message_remains_auditable_after_review(self):
        original = self.connection.execute(
            "SELECT message FROM outreach_drafts WHERE id=1"
        ).fetchone()["message"]
        lead_cli.review_draft(self.connection, 1, "REJECTED")
